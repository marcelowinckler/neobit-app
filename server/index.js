...
function generateHTMLFromPrompt({ prompt, baseHtml, provider, mode }) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (wantsTailwind) {
        systemPrompt = 'Você é um gerador de landing pages modernas com estética premium. Sua tarefa é criar código HTML visualmente impressionante usando EXCLUSIVAMENTE Tailwind CSS.\n\nREGRAS OBRIGATÓRIAS:\n1. USE APENAS Tailwind CSS via CDN (<script src="https://cdn.tailwindcss.com"></script>). NÃO escreva CSS puro em tags <style>.\n2. O design deve ser moderno, clean e profissional (estilo SaaS, startup ou premium).\n3. Use gradientes suaves, sombras sutis (shadow-lg, shadow-xl), bordas arredondadas (rounded-2xl, rounded-full) e espaçamento generoso.\n4. Seção Hero deve ser impactante, com tipografia grande e clara.\n5. Use cores harmoniosas. Se não especificado, use tons de azul/indigo ou preto/branco minimalista.\n6. Adicione seções completas: Header, Hero, Features, Testimonials, Pricing (se aplicável), Footer.\n7. NÃO use imagens quebradas. Use emojis ou ícones SVG inline se necessário, ou placeholders coloridos (divs com bg-color).\n8. O código deve ser 100% funcional e responsivo (mobile-first).\n9. Responda APENAS com o código HTML dentro de blocos ```html```. NÃO explique o código.\n10. NÃO coloque placeholders como "Conteúdo aqui". Preencha com texto fictício realista (Lorem Ipsum ou texto em português convincente).'
        userContent = `Crie uma landing page profissional para: ${prompt}. Capriche no design moderno e use Tailwind CSS.`
      } else if (isEdit) {
        systemPrompt = 'Você é um especialista em web design e modificação de código. \n\nREGRAS CRÍTICAS PARA EDIÇÃO:\n1. Se você receber um HTML de base, sua prioridade absoluta é MANTER O CÓDIGO ORIGINAL e aplicar APENAS a mudança solicitada.\n2. NÃO REESCREVA o site do zero. NÃO ALTERE estilos, cores ou layouts que não foram mencionados.\n3. Copie o código original e modifique apenas as linhas necessárias para atender ao pedido.\n4. Se o usuário pedir explicitamente um NOVO site ("crie", "gere"), aí sim você pode ignorar o base.\n\nResponda APENAS com o código HTML final dentro de ```html```. NÃO explique.'
        ...
        systemPrompt = 'Você é um gerador de sites experiente. Sua tarefa é criar um NOVO site moderno e bonito baseado no pedido do usuário. \n\nREGRAS DE CRIAÇÃO:\n1. Use EXCLUSIVAMENTE Tailwind CSS via CDN. Não use CSS puro.\n2. Design deve ser visualmente rico, não apenas texto preto no branco.\n3. Preencha todo o conteúdo, não deixe placeholders vazios.\n4. Use seções bem definidas (Hero, Features, Footer).\n5. Responda APENAS com o código HTML dentro de blocos ```html```.\n\nResponda APENAS com o HTML completo.'
        userContent = baseHtml ? `PEDIDO DO USUÁRIO: ${prompt}\n\n(Opcional) HTML DE REFERÊNCIA/TEMPLATE:\n${baseHtml}` : `Crie um novo site moderno e completo para: ${prompt}. Use Tailwind CSS.`
      }
      ...
      if (!/</.test(html)) html = `<!DOCTYPE html><html><head><meta charset=\"utf-8\" /><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\" /><meta http-equiv=\"Content-Security-Policy\" content=\"upgrade-insecure-requests\" />${wantsTailwind ? '<script src=\\\"https://cdn.tailwindcss.com\\\"></script>' : ''}</head><body>${html.replace(/[&<>]/g, s => ({'&':'&amp;','<':'&lt;','>':'&gt;'}[s]))}</body></html>`
